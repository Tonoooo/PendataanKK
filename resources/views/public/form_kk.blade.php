...
<div class="flex flex-col min-h-screen">
    ...
    <main class="flex-grow py-12 sm:py-16">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="max-w-4xl mx-auto bg-white p-6 sm:p-8 rounded-2xl shadow-lg">
                ...
                <h1 class="text-3xl font-bold text-center text-gray-900 mb-2">Formulir Pembuatan</h1>
                <h1 class="text-xl font-bold text-center text-gray-900 mb-2">Data Ikhwan Jabar</h1>
                ...
                <form action="{{ route('kk.store') }}" method="POST" class="space-y-8">
                    ...
                    <fieldset class="space-y-4 border-t pt-6">
                        <legend class="text-xl font-semibold text-gray-800">Data Alamat Ikhwan</legend>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                            <div class="sm:col-span-2"><x-input-label for="nama_kepala_keluarga" value="Nama Ikhwan" /><x-text-input id="nama_kepala_keluarga" name="nama_kepala_keluarga" type="text" class="mt-1 block w-full" :value="old('nama_kepala_keluarga')"  /></div>
                            ...
                            <div><x-input-label for="kecamatan" value="Riyasah" /><x-text-input id="kecamatan" name="kecamatan" type="text" class="mt-1 block w-full" :value="old('kecamatan')" required /></div>
                            <div><x-input-label for="kabupaten_kota" value="Niyabah" /><x-text-input id="kabupaten_kota" name="kabupaten_kota" type="text" class="mt-1 block w-full" :value="old('kabupaten_kota')" required /></div>
                            <div><x-input-label for="provinsi" value="Wilayah" /><x-text-input id="provinsi" name="provinsi" type="text" class="mt-1 block w-full" :value="old('provinsi')" required /></div>
                        </div>
                    </fieldset>
                    ...
                    <fieldset class="space-y-4 border-t pt-6" x-data="{ anggota: {{ json_encode(old('anggota', [['status_hubungan_dalam_keluarga' => 'KEPALA KELUARGA', 'jenis_kelamin' => 'LAKI-LAKI', 'status_perkawinan' => 'BELUM KAWIN', 'kewarganegaraan' => 'WNI']])) }} }">
                        <legend class="text-xl font-semibold text-gray-800">Biodata Ikhwan</legend>
                        <div class="space-y-6">
                                ...
                                <div class="border border-gray-200 p-4 rounded-lg relative space-y-4 bg-gray-50/50">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
                                        ...
                                        <div class="lg:col-span-3">
                                            ...
                                            <input x-bind:id="`anggota_nama_${index}`" x-bind:name="`anggota[${index}][nama_lengkap]`" type="text" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full" x-model="orang.nama_lengkap" >
                                        </div>
                    
                                        <div>
                                            <label x-bind:for="`anggota_nik_${index}`" class="block font-medium text-sm text-gray-700">NIK</label>
                                            <input x-bind:id="`anggota_nik_${index}`" x-bind:name="`anggota[${index}][nik]`" type="number" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full" x-model="orang.nik" >
                                        </div>
                    
                                        <div>
                                            <label x-bind:for="`anggota_jk_${index}`" class="block font-medium text-sm text-gray-700">Jenis Kelamin</label>
                                            <select x-bind:id="`anggota_jk_${index}`" x-bind:name="`anggota[${index}][jenis_kelamin]`" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full" x-model="orang.jenis_kelamin" >
                                        ...
                                        <div>
                                            ...
                                            <input x-bind:id="`anggota_hubungan_${index}`" x-bind:name="`anggota[${index}][status_hubungan_dalam_keluarga]`" type="text" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full" x-model="orang.status_hubungan_dalam_keluarga" >
                                        </div>
                    
                                        {{-- Menggunakan tag HTML standar untuk semua input dinamis --}}
                                        <div><label class="block font-medium text-sm text-gray-700">Tempat Lahir</label><input x-bind:name="`anggota[${index}][tempat_lahir]`" type="text" x-model="orang.tempat_lahir"  class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full"></div>
                                        <div><label class="block font-medium text-sm text-gray-700">Tanggal Lahir</label><input x-bind:name="`anggota[${index}][tanggal_lahir]`" type="date" x-model="orang.tanggal_lahir"  class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full"></div>
                                        <div><label class="block font-medium text-sm text-gray-700">Agama</label><input x-bind:name="`anggota[${index}][agama]`" type="text" x-model="orang.agama"  class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full"></div>
                                        <div><label class="block font-medium text-sm text-gray-700">Pendidikan Terakhir</label><input x-bind:name="`anggota[${index}][pendidikan]`" type="text" x-model="orang.pendidikan"  class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full"></div>
                                        <div><label class="block font-medium text-sm text-gray-700">Jenis Pekerjaan</label><input x-bind:name="`anggota[${index}][jenis_pekerjaan]`" type="text" x-model="orang.jenis_pekerjaan"  class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full"></div>
                                        ...
                                        <div>
                                            ...
                                            <select x-bind:name="`anggota[${index}][status_perkawinan]`" x-model="orang.status_perkawinan"  class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full"><option>BELUM KAWIN</option><option>KAWIN</option><option>CERAI HIDUP</option><option>CERAI MATI</option></select>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <select x-bind:name="`anggota[${index}][kewarganegaraan]`" x-model="orang.kewarganegaraan"  class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full"><option>WNI</option><option>WNA</option></select>
                                        </div>
                                        <div><label class="block font-medium text-sm text-gray-700">Nama Ayah</label><input x-bind:name="`anggota[${index}][nama_ayah]`" type="text" x-model="orang.nama_ayah"  class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full"></div>
                                        <div><label class="block font-medium text-sm text-gray-700">Nama Ibu</label><input x-bind:name="`anggota[${index}][nama_ibu]`" type="text" x-model="orang.nama_ibu"  class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 w-full"></div>
    ...
    <footer class="bg-white border-t border-gray-200 mt-auto">
        <div class="container mx-auto py-6 px-4 sm:px-6 lg:px-8">
            <p class="text-center text-sm text-gray-500">© {{ date('Y') }} Data Ikhwan Jabar. All rights reserved.</p>
        </div>
    </footer>
